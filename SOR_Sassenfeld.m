...
x0 = [0; 0; 0];
tol = 0.05;
omega = 1.2;  % Fator de relaxação

fprintf("Utilizando matriz da questão 1 (SOR - ω = %.2f)\n", omega);
...
while err > tol
    ...
    % Atualizações com fator de relaxação ω
    x(1) = (1 - omega)*x_ant(1) + (omega/A(1,1))*(b(1) - A(1,2)*x_ant(2) - A(1,3)*x_ant(3));
    x(2) = (1 - omega)*x_ant(2) + (omega/A(2,2))*(b(2) - A(2,1)*x(1) - A(2,3)*x_ant(3));
    x(3) = (1 - omega)*x_ant(3) + (omega/A(3,3))*(b(3) - A(3,1)*x(1) - A(3,2)*x(2));
