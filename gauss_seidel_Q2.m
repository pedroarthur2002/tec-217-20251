...
x0 = [0; 0; 0; 0];
tol = 0.05;
fprintf("Utilizando sistema 4x4 (Gauss-Seidel)\n");
...
fprintf("\nIteração\t   x(1)\t\t   x(2)\t\t   x(3)\t\t   x(4)\t\tErro relativo\n");
while err > tol
    x_ant = x;
    x(1) = (b(1) - A(1,2)*x_ant(2) - A(1,3)*x_ant(3) - A(1,4)*x_ant(4)) / A(1,1);
    x(2) = (b(2) - A(2,1)*x(1) - A(2,3)*x_ant(3) - A(2,4)*x_ant(4)) / A(2,2);
    x(3) = (b(3) - A(3,1)*x(1) - A(3,2)*x(2) - A(3,4)*x_ant(4)) / A(3,3);
    x(4) = (b(4) - A(4,1)*x(1) - A(4,2)*x(2) - A(4,3)*x(3)) / A(4,4);

    itr = itr + 1;
    err = max(abs((x - x_ant) ./ x));
    fprintf("%5d\t\t%8.5f\t%8.5f\t%8.5f\t%8.5f\t%e\n", itr, x(1), x(2), x(3), x(4), err);
